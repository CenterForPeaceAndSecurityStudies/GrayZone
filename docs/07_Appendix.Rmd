...
NATO membership is a crude observable measure of alliances that proxy for deterrence. We use a series of alternate measures of alliances as deterrence here. We run the new models using the relevant states sample used in models 4-6 in the main text.
```{r}
df <- df %>%
...
### NATO duration
Instead of NATO membership as a dummy, we use years of NATO membership to account for intra-alliance variation in extended deterrence. Those results are shown in \@ref(tab:alt1-results).

```{r}
df_m1avars <- df %>%
  dplyr::select(intensity, NATOdur_MEM, lnmindistkm_rus, dplyr::starts_with("year_"))
...
m1a <- lmtest::coeftest(m1a, vcov = sandwich::vcovCL(m1a, factor(df$cname1)))
```

```{r}
df_m1bvars <- df %>%
  dplyr::select(intensity, NATOdur_MEM, lnmindistkm_rus, demo1, nuclear1, gdppc1_2010const, lnpop1, dplyr::starts_with("year_")) %>%
  dplyr::mutate(gdppc = gdppc1_2010const/1000) %>%
...
# Country-clustered SE
m1b <- lmtest::coeftest(m1b, vcov = sandwich::vcovCL(m1b, factor(df$cname1)))
```

```{r}
df_m1cvars <- df %>%
  dplyr::select(intensity, NATOdur_MEM, lnmindistkm_rus, demo1, nuclear1, gdppc1_2010const, lnpop1, milex_sipri, dplyr::starts_with("year_")) %>%
  dplyr::mutate(gdppc = gdppc1_2010const/1000) %>%
  dplyr::mutate(milex = milex_sipri/1000) %>%
  dplyr::select(-gdppc1_2010const, -milex_sipri)
...
# Country-clustered SE
m1c <- lmtest::coeftest(m1c, vcov = sandwich::vcovCL(m1c, factor(df$cname1)))
```

```{r}
models <- list(m1a, m1b, m1c)

texreg::screenreg(models,
               stars = c(0.01, 0.05, 0.1),
               omit.coef = "(y)",
               custom.coef.names = c("NATO member duration",
                                     "Russia distance",
                                     "Democracy",
                                     "Nuclear power",
                                     "Population",
               ...
               custom.gof.rows = list("Observations" = c("376", "373", "346")), 
               custom.note = "All models include year-fixed effects with country-clustered standard errors in parentheses. %stars",
               float.pos = "h!",
               label="tab:alt1-results",
               caption = "Intensity of Russian Intervention: Alternate deterrence variable",
               use.packages = FALSE)
```

### NATO stage
We here account for pre-NATO membership by using an ordered factor variable for the stages of NATO membership; non-member, Partnership for Peace (PfP), Intensive Dialogue, Membership Action Plan (MAP), and full member. Those results are shown in \@ref(tab:alt2-results).

```{r}
df_m2avars <- df %>%
  dplyr::select(intensity, nato_stage, lnmindistkm_rus, dplyr::starts_with("year_"))
...
# Country-clustered SE
m2a <- lmtest::coeftest(m2a, vcov = sandwich::vcovCL(m2a, factor(df$cname1)))
```

```{r}
df_m2bvars <- df %>%
  dplyr::select(intensity, nato_stage, lnmindistkm_rus, demo1, nuclear1, gdppc1_2010const, lnpop1, dplyr::starts_with("year_")) %>%
  dplyr::mutate(gdppc = gdppc1_2010const/1000) %>%
  dplyr::select(-gdppc1_2010const)
...
# Country-clustered SE
m2b <- lmtest::coeftest(m2b, vcov = sandwich::vcovCL(m2b, factor(df$cname1)))
```

```{r}
df_m2cvars <- df %>%
  dplyr::select(intensity, nato_stage, lnmindistkm_rus, demo1, nuclear1, gdppc1_2010const, lnpop1, milex_sipri, dplyr::starts_with("year_")) %>%
  dplyr::mutate(gdppc = gdppc1_2010const/1000) %>%
  dplyr::mutate(milex = milex_sipri/1000) %>%
  dplyr::select(-gdppc1_2010const, -milex_sipri)
...
# Country-clustered SE
m2c <- lmtest::coeftest(m2c, vcov = sandwich::vcovCL(m2c, factor(df$cname1)))
```

```{r}
models <- list(m2a, m2b, m2c)
...
texreg::screenreg(models,
               ...
               custom.note = "All models include year-fixed effects with country-clustered standard errors in parentheses. %stars",
               float.pos = "h!",
               label="tab:alt2-results",
               caption = "Intensity of Russian Intervention: Alternate deterrence variable",
               use.packages = FALSE)
```

### DCAs
We here use defense cooperation agreements (DCAs) as an alternate measure of alliance ties produced by @kinne_defensecooperationagreement_2020. Those results are shown in \@ref(tab:alt3-results).

```{r}
df_m3avars <- df %>%
  dplyr::select(intensity, dca, lnmindistkm_rus, dplyr::starts_with("year_"))
...
# Country-clustered SE
m3a <- lmtest::coeftest(m3a, vcov = sandwich::vcovCL(m3a, factor(df$cname1)))
```

```{r}
df_m3bvars <- df %>%
  dplyr::select(intensity, dca, lnmindistkm_rus, demo1, nuclear1, gdppc1_2010const, lnpop1, dplyr::starts_with("year_")) %>%
  dplyr::mutate(gdppc = gdppc1_2010const/1000) %>%
  dplyr::select(-gdppc1_2010const)
...
# Country-clustered SE
m3b <- lmtest::coeftest(m3b, vcov = sandwich::vcovCL(m3b, factor(df$cname1)))
```

```{r}
df_m3cvars <- df %>%
  dplyr::select(intensity, dca, lnmindistkm_rus, demo1, nuclear1, gdppc1_2010const, lnpop1, milex_sipri, dplyr::starts_with("year_")) %>%
  dplyr::mutate(gdppc = gdppc1_2010const/1000) %>%
  dplyr::mutate(milex = milex_sipri/1000) %>%
  dplyr::select(-gdppc1_2010const, -milex_sipri)
...
# Country-clustered SE
m3c <- lmtest::coeftest(m3c, vcov = sandwich::vcovCL(m3c, factor(df$cname1)))
```

```{r}
models <- list(m3a, m3b, m3c)

texreg::screenreg(models,
               stars = c(0.01, 0.05, 0.1),
               omit.coef = "(y)",
               custom.coef.names = c("Defense Cooperation Agreement",
                                     "Russia distance",
                                     "Democracy",
                                     "Nuclear power",
                                     "Population",
               ...
               custom.note = "All models include year-fixed effects with country-clustered standard errors in parentheses. %stars",
               float.pos = "h!",
               label="tab:alt3-results",
               caption = "Intensity of Russian Intervention: Alternate deterrence variable",
               use.packages = FALSE)
```

### US troop presence
We here use counts of the number of US troops from @allen_globalmilitarydeployment_2021. Those results are shown in \@ref(tab:alt4-results).
...
# Country-clustered SE
m4a <- lmtest::coeftest(m4a, vcov = sandwich::vcovCL(m4a, factor(df$cname1)))
```
...
# Country-clustered SE
m4b <- lmtest::coeftest(m4b, vcov = sandwich::vcovCL(m4b, factor(df$cname1)))
```
...
# Country-clustered SE
m4c <- lmtest::coeftest(m4c, vcov = sandwich::vcovCL(m4c, factor(df$cname1)))
```

```{r}
models <- list(m4a, m4b, m4c)

texreg::screenreg(models,
               stars = c(0.01, 0.05, 0.1),
               omit.coef = "(y)",
               custom.coef.names = c("US troop presence",
                                     "Russia distance",
                                     "Democracy",
                                     "Nuclear power",
                                     "Population",
               ...
               custom.note = "All models include year-fixed effects with country-clustered standard errors in parentheses. %stars",
               float.pos = "h!",
               label="tab:alt4-results",
               caption = "Intensity of Russian Intervention: Alternate deterrence variable",
               use.packages = FALSE)
```

### US bases
We similarly use counts of US bases in each state, also from @allen_globalmilitarydeployment_2021. Those results are shown in \@ref(tab:alt5-results).

```{r}
df_m5avars <- df %>%
  dplyr::select(intensity, usbases_main, lnmindistkm_rus, dplyr::starts_with("year_"))
...
# Country-clustered SE
m5a <- lmtest::coeftest(m5a, vcov = sandwich::vcovCL(m5a, factor(df$cname1)))
```

```{r}
df_m5bvars <- df %>%
  dplyr::select(intensity, usbases_main, lnmindistkm_rus, demo1, nuclear1, gdppc1_2010const, lnpop1, dplyr::starts_with("year_")) %>%
  dplyr::mutate(gdppc = gdppc1_2010const/1000) %>%
  dplyr::select(-gdppc1_2010const)
...
# Country-clustered SE
m5b <- lmtest::coeftest(m5b, vcov = sandwich::vcovCL(m5b, factor(df$cname1)))
```

```{r}
df_m5cvars <- df %>%
  dplyr::select(intensity, usbases_main, lnmindistkm_rus, demo1, nuclear1, gdppc1_2010const, lnpop1, milex_sipri, dplyr::starts_with("year_")) %>%
  dplyr::mutate(gdppc = gdppc1_2010const/1000) %>%
  dplyr::mutate(milex = milex_sipri/1000) %>%
  dplyr::select(-gdppc1_2010const, -milex_sipri)
...
# Country-clustered SE
m5c <- lmtest::coeftest(m5c, vcov = sandwich::vcovCL(m5c, factor(df$cname1)))
```

```{r}
models <- list(m5a, m5b, m5c)

texreg::screenreg(models,
               stars = c(0.01, 0.05, 0.1),
               omit.coef = "(y)",
               custom.coef.names = c("US bases hosted",
                                     "Russia distance",
                                     "Democracy",
                                     "Nuclear power",
                                     "Population",
