...
A comparison of what cases were covered in each individual dataset is provided here. Note that there significant inconsistency concerning the sample of post-1994 Russian interventions identified by the ICB, DCID, and REI datasets:
...
stargazer::stargazer(df_vars, 
                     ...
                     covariate.labels = c("Intensity", "NATO member", "Dist. from Russia (minimum, log)", "Democracy", "Nuclear state", "GDP per capita", "Population (log)", "Military Expenditure", "CINC ratio"),
                     digits = 1,
...
The bivariate correlations between the DV and the two EVs are shown below. The intensity and NATO variables have been converted to numeric values to simplify visualizing the bivariate correlations. The intensity scale representing the dependent variable (the x-axis) appears to go to 6 instead of 5 because the ordinal value of 0 is numerically coded as 1, so all dependent variable values are shifted by one in this simplified depiction.
...
## Odds ratio
The table of results in the main text shows coefficients from the ordinal probit model. Since those are difficult to interpret, the results as odds ratios are shown here:
...
# Model 2
m1_imp <- Hmisc::fit.mult.impute(intensity ~ 
                                   NATOmem_MEM +
                                   lnmindistkm_rus,
                                 fitter = rms::lrm, 
                                 xtrans = df_imp,
...
m1_imp <- rms::robcov(m1_imp, df$cabbrev1)
```
...
```{r model 4}
# Model 4
m4_imp <- Hmisc::fit.mult.impute(intensity ~ 
                                   NATOmem_MEM +
                                   lnmindistkm_rus,
                                 fitter = rms::lrm, 
...
# Country-clustered SE
m4_imp <- rms::robcov(m4_imp, df_conserv$cabbrev1)
```
...
models_imp <- list(m1_imp, m2_imp, m3_imp, m4_imp, m5_imp, m6_imp)
