...
continent <- democracyData::country_year_coder(df, 
                                  code_col = ccode1,
                                  date_col = year, 
                                  code_type = "cown", 
                                  match_type = "code only",
                                  to_system = "cow", 
                                  include_in_output = c("official_continent")) %>%
  dplyr::rename(continent = official_continent) %>%
  dplyr::select(year, cabbrev1, ccode1, continent)

continent$continent[continent$cabbrev1 == "YUG" | continent$cabbrev1 == "KOS" | continent$cabbrev1 == "GRG"] <- "Europe"

continent <- continent %>%
  dplyr::distinct()

df <- dplyr::left_join(df, continent)
...
# Data summary
That leaves us with a dataset where the unit of analysis is the country-year, the universe of cases is country-years that were the targets of Russian intervention from 1994-2017, and the dependent variable is the means of Russian intervention
```{r}
DT::datatable(df)
naniar::gg_miss_var(df)
