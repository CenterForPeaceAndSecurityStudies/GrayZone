...
```{r}
# Create dataframe versions of just variables to merge for the match
cpass_join <- cpass[ , c('Target', 'Year', 'cpass', 'det_alliance', 'resp_convmil', 'resp_convmil_gro', 'resp_convmil_airsea', 'resp_paramil', 'resp_cyberdisrup', 'resp_infoops')]
...
```{r}
saveRDS(cases_old, paste0(here::here(), "/data/","grayzone_old.rds"))
write.csv(cases_old, paste0(here::here(), "/data/","grayzone_old.csv"))
```


# Reload dataframe with new variables coded
The new variables for intensity have been coded by hand into the previous spreadsheet. That can be reloaded now abd nerged with the new cases that were similarly coded to analyze the complete version of the dataset
...
#Save
#saveRDS(df_complete, paste0(here::here(), "/data/","grayzone_aggregate_cpass.rds"))
```
